...
#         return out
class ResidualAttentionBlock(nn.Module):
    def __init__(self, channel, use_rgca=True, use_dpca=True):
        super(ResidualAttentionBlock, self).__init__()

        self.use_rgca = use_rgca
    ...
    def forward(self, x):
        # 如果整个 RGCA 都不要，直接退化为 identity
        if not self.use_rgca:
            return x

        y = self.conv(x)
        y = self.relu(y)
        y = self.attn(y)   # 如果 w/o DPCA，这里就是 Identity
        out = x + y
        return out
